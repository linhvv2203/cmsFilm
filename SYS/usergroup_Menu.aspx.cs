...
public partial class SYS_usergroup_Menu : System.Web.UI.Page
{
    ...
    private void BindDataControl()
    {
        ...
        SQL = "CategoriesMenu WHERE ParentID=0 and CategoryStatus=2";
        ...
        ddlCategory.SelectedValue = "0";//.Items.FindByValue("0").Selected = true;
    ...
    private void getAllChecked()
    {
        ...
        {
            sTemp = "SELECT UserGroupID from viwRightsMenuUserGroups WHERE MenuID=" + dgrCommon.DataKeys[i] + " AND UserGroupID=" + sID + "AND RightsStatus=1";
    ...
    protected void cmdSave_Click(object sender, System.EventArgs e)
    {
        ...
        for (i = 0; i < dgrCommon.Items.Count; i++)
        {
            ...
            if (((CheckBox)dgrCommon.Items[i].FindControl("chkAllDelete")).Checked == true)
            {
                ...
                string SQL = "SELECT MenuID from RightsMenu WHERE UserGroupID=" + sID + " And MenuID=" + skey;
                ...
                if (al.Count == 0)
                {
                    string mSQL = "INSERT INTO RightsMenu(UserGroupID,MenuID,RightsStatus) VALUES(" + sID + "," + skey + ",1)";
        ...
        if (sTemp != "")
        {
            SQL = "UPDATE RightsMenu SET RightsStatus=1 WHERE UserGroupID=" + sID + " AND MenuID in (" + sTemp + ")";
        ...
        if (sTemp != "")
        {
            SQL = "UPDATE RightsMenu SET RightsStatus=0 WHERE UserGroupID=" + sID + " AND MenuID in (" + sTemp + ")";
            DAL.ExecuteQuery(SQL);
            VatLid.DAL.INSERT_USER_LOG_NEW(VatLid.Utils.getUserId(Session).ToString(), VatLid.LogType.UPDATE.ToString(), VatLid.DAL.getCategoryID(FileName), "OK", sTemp, VatLid.Utils.GetIP());
        }
        BindData();
