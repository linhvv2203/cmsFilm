...
public partial class OnKeeng_ListMovieStatistics : System.Web.UI.Page
{
    ...
    private DataSet BuildFilter()
    {
        ...
        try
        {
            ...
            ds = VatLidOnPhim.DAL.GetDataSet("cms_ItemMovie_Statistics", cmd);
        }
        catch
        {
            return null;
    ...
    {
        if(ddlPartnerID.SelectedValue == "0")
        {
            lblError.Text = "Bạn cần chọn tên đối tác.";
            return;
        }
        if(string.IsNullOrEmpty(txtDateStart.Value))
        {
            lblError.Text = "Bạn cần chọn DateStart.";
            return;
        }
        if (string.IsNullOrEmpty(txtDateEnd.Value))
        {
            lblError.Text = "Bạn cần chọn tên DateEnd.";
            return;
        }
        BindDataGrid();
